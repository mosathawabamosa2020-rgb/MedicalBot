...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    ...
    if (contentType.includes('pdf')) {
      ...
      const filePath = path.join(uploadsDir, `${created.id}.pdf`)
      try {
        fs.writeFileSync(filePath, buffer)
        await prisma.reference.update({ where: { id: created.id }, data: { filePath } })
      } catch (e: unknown) {
    ...
    const filePath = path.join(uploadsDir, `${created.id}.html`)
    try {
