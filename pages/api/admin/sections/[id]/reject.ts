...
// simple in-memory rate limiter per user (shared with verify)
...
async function handler(req: NextApiRequest, res: NextApiResponse, session?: any) {
  ...
  try {
    ...
    await prisma.section.update({ where: { id }, data: { status: 'rejected' } })
    await (prisma as any).sectionAuditLog.create({ data: { sectionId: id, userId: user.id, previousStatus: prev.status as any, newStatus: 'rejected' } })
...
// wrap handler in admin authorization
