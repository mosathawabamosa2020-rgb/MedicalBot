# REPORTS_INDEX

Date: 2026-03-12

## Current Canonical Reports
- docs/00-governance/SOURCE_OF_TRUTH.md
- docs/00-governance/CURRENT_PROJECT_STATUS.md
- docs/01-architecture/KNOWLEDGE_PIPELINE_TRUTH_MAP.md
- docs/01-architecture/FILE_STORAGE_ARCHITECTURE_REVIEW.md
- docs/01-architecture/RETRIEVAL_ARCHITECTURE_AND_DIMENSION_VALIDATION.md
- docs/01-architecture/SCHEMA_SEMANTICS_REVIEW.md
- docs/01-architecture/PYTHON_AND_WORKER_GOVERNANCE_REVIEW.md
- docs/01-architecture/OPERATOR_EXPERIENCE_AND_UI_REVIEW.md
- docs/01-architecture/SOURCE_REGISTRY_EXPANSION_REVIEW.md
- docs/02-validation/PLATFORM_VALIDATION_REPORT.md
- docs/02-validation/CUMULATIVE_EXECUTION_AND_VERIFICATION_REPORT.md
- docs/02-validation/BUILD_DETERMINISM_ROOT_CAUSE.md
- docs/02-validation/FINAL_RUNTIME_ROUTE_VERIFICATION.md
- docs/02-validation/CORE_VALUE_CHAIN_ASSESSMENT.md
- docs/02-validation/OPERATIONAL_READINESS_ASSESSMENT.md
- docs/02-validation/IMPLEMENTATION_EXECUTION_REPORT_GROUP_D.md
- docs/02-validation/IMPLEMENTATION_EXECUTION_REPORT_GROUP_E.md
- docs/02-validation/PLATFORM_DEEP_VERIFICATION_AND_ALERTING_REPORT.md
- docs/02-validation/PLATFORM_LIVE_MULTI_SOURCE_PROOF_REPORT.md
- docs/03-operations/backup-restore-runbook.md
- docs/03-operations/readiness-model.md

## Historical / Superseded Material
- Timestamped reports belong under docs/archive/historical/.
- Obsolete and replaced reports belong under docs/archive/superseded/.
