...
describe('runIngestionWorker', () => {
    ...
    test('processes reference with full text', async () => {
        ...
        expect(prisma.reference.update).toHaveBeenCalledWith({ where: { id: 'r1' }, data: { status: 'processed', processingDate: expect.any(Date) } });
        expect(prisma.ingestionLog.create).toHaveBeenCalledWith(expect.objectContaining({ data: expect.objectContaining({ message: 'Successfully processed reference r1', referenceId: 'r1' }) }));
    });
    test('handles adapter failure gracefully', async () => {
        ...
        PubMedAdapter.mockImplementation(() => ({ fetchFullText: fetchMock }));
        await runIngestionWorker();
        expect(prisma.reference.update).toHaveBeenCalledWith({ where: { id: 'r2' }, data: { status: 'processed_no_full_text', processingDate: expect.any(Date) } });
