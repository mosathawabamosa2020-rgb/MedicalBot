...
function Research() {
    ...
    const [deviceId, setDeviceId] = (0, react_1.useState)('');
    const [imported, setImported] = (0, react_1.useState)({});
    ...
    async function importArticle(item) {
        if (!deviceId)
            return alert('select device id (device must exist)');
        ...
        const j = await r.json();
        if (r.status === 201) {
            setImported(prev => (Object.assign(Object.assign({}, prev), { [item.id]: true })));
            (0, swr_1.mutate)('/api/admin/stats');
    ...
    return (<main className="p-6">
      ...
      <ul>
        {results.map(r => (<li key={r.id} className="mb-3 p-3 border rounded">
            <div className="font-semibold">{r.title}</div>
            <div className="text-sm text-gray-600">{(r.authors || []).join(', ')}</div>
            <div className="text-sm text-gray-500">{r.pubDate}</div>
            <div className="mt-2">
              <button className="mr-2 bg-green-600 text-white px-3 py-1 rounded disabled:opacity-50" onClick={() => importArticle(r)} disabled={!!imported[r.id]}>
