...
function ensureDataFiles() {
    ...
    if (!fs.existsSync(STATE_PATH))
        fs.writeFileSync(STATE_PATH, JSON.stringify({ processedDetails: [], lastSearch: null, lastPageUrl: null }, null, 2));
}
function loadState() {
    ensureDataFiles();
...
function ensureEmbedLib() {
    if (!embedLib)
        embedLib = require(path.join(process.cwd(), 'dist', 'lib', 'embeddings'));
    return embedLib;
}
function cosine(a, b) {
    let dot = 0, na = 0, nb = 0;
...
async function runOnce(searchTerm) {
    ...
    while (true) {
        ...
        if (fs.existsSync(STOP_FLAG)) {
            console.log('Stop flag detected; exiting');
            break;
        }
        ...
        for (const detailUrl of detailUrls) {
            if (state.processedDetails && state.processedDetails.includes(detailUrl)) {
                continue;
            }
            try {
                ...
                if (!resp.ok) {
                    console.warn('Failed to fetch detail', resp.status);
                    continue;
                }
                ...
                if (pdfUrl) {
                    try {
                        ...
                        console.warn('PDF download error', e.message);
                    }
                }
                state.processedDetails.push(detailUrl);
                saveState(state);
