...
async function run() {
    try {
        ...
        if (!apiKey) {
            console.error('GOOGLE API key required as CLI arg or GOOGLE_API_KEY env');
            process.exit(2);
        }
        const chunksPath = path.join(process.cwd(), 'data', 'chunks_to_embed.json');
        if (!fs.existsSync(chunksPath)) {
            console.error('chunks_to_embed.json not found');
            process.exit(3);
        }
        ...
        for (const c of chunks) {
            try {
                ...
                let emb = null;
                if (j && Array.isArray(j.embeddings) && j.embeddings[0] && Array.isArray(j.embeddings[0].embedding))
                    emb = j.embeddings[0].embedding;
                if (!emb && j && j.results && j.results[0] && j.results[0].embedding)
                    emb = j.results[0].embedding;
                if (!emb && j && j.data && j.data[0] && j.data[0].embedding)
                    emb = j.data[0].embedding;
                if (!emb) {
                    console.warn('NO_EMBED_IN_RESP', c.id, JSON.stringify(j).slice(0, 200));
                    continue;
                }
                ...
                console.error('ERR_EMBED', c.id, e && e.message || e);
            }
            // small delay
